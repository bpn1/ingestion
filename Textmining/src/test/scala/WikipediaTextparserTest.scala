	...
	def wikipediaTestReferences(): Map[String, List[WikipediaTextparser.Link]] = {
		Map(
			...
			"Abraham Lincoln" -> List(
				// Article text links
				WikipediaTextparser.Link("President of the United States", "President of the United States", 29),

				// Infobox links
				WikipediaTextparser.Link("Hannibal Hamlin", "Hannibal Hamlin", WikipediaTextparser.infoboxOffset),
				WikipediaTextparser.Link("Andrew Johnson", "Andrew Johnson", WikipediaTextparser.infoboxOffset)))
	}
