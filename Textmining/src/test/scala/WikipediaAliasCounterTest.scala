...
class WikipediaAliasCounterTest extends FlatSpec with SharedSparkContext {
	...
	def aliasOccurrencesInArticlesTestRDD(): RDD[WikipediaAliasCounter.AliasOccurrencesInArticle] = {
		sc.parallelize(List(
			WikipediaAliasCounter.AliasOccurrencesInArticle(Set("Audi"), Set()),
			WikipediaAliasCounter.AliasOccurrencesInArticle(Set(), Set("Audi")),
			WikipediaAliasCounter.AliasOccurrencesInArticle(Set("Brachttal", "Main-Kinzig-Kreis", "Hessen", "1377", "Büdinger Wald"), Set("Streitberg")),
			WikipediaAliasCounter.AliasOccurrencesInArticle(Set("Audi", "Brachttal", "historisches Jahr"), Set("Hessen", "Main-Kinzig-Kreis", "Büdinger Wald", "Backfisch"))
		))
	}
	...
	def countedAliasesTestRDD(): RDD[WikipediaAliasCounter.AliasCounter] = {
		sc.parallelize(List(
			...
			WikipediaAliasCounter.AliasCounter("Streitberg", 0, 1),
			WikipediaAliasCounter.AliasCounter("historisches Jahr", 1, 1)
		))
	}
}
